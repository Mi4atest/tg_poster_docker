...
def format_for_telegram_plain(text: str, signature_enabled: bool = True) -> str:
    """
    Текст для Telegram без parse_mode: структура как у поста, но без Markdown-разметки и экранирования.
    Используется при fallback-редактировании (когда MARKDOWN_V2 недоступен), чтобы не показывать \\.
    """
    formatted_text = format_post_text(text)
    formatted_text = re.sub(r'—{30}', '—' * 19, formatted_text)
    formatted_text += get_telegram_signature(enabled=signature_enabled)
    # Убираем маркеры жирного/курсива — без parse_mode они отображались бы как есть
...
def format_for_max(text: str, signature_enabled: bool = True) -> str:
    """Форматирует текст для Max markdown без Telegram-экранирования.

    Max markdown ожидает **bold**, а не telegram-style *bold*.
    """
    formatted_text = format_post_text(text)
    formatted_text = re.sub(r'—{30}', '—' * 19, formatted_text)
    formatted_text += get_telegram_signature(enabled=signature_enabled)
...
def format_for_max_plain(text: str, signature_enabled: bool = True) -> str:
    """Fallback-текст для Max без parse_mode."""
    return format_for_telegram_plain(text, signature_enabled=signature_enabled)
