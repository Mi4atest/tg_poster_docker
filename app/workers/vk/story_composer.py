"""Сборка кадра истории VK 9:16 в стиле share-to-story (blur + белый бабл)."""
...
def _layout_cells(count: int, area: Tuple[int, int, int, int]) -> List[Tuple[int, int, int, int]]:
...
def build_story_image(
    ...
    *,
    ...
    """
    JPEG 1080x1920: blur-фон из первого фото, белый бабл (лого + коллаж 6 фото + модель/цена).
    """
    _ = footer_text
    ...
    title_h = sum(probe.textbbox((0, 0), ln, font=title_font)[3] - probe.textbbox((0, 0), ln, font=title_font)[1] + 6 for ln in title_lines)
    price_h = sum(probe.textbbox((0, 0), ln, font=price_font)[3] - probe.textbbox((0, 0), ln, font=price_font)[1] + 4 for ln in price_lines)
    caption_block_h = 0
    if title_lines or price_lines:
        caption_block_h = caption_gap + title_h + (10 if title_lines and price_lines else 0) + price_h + pad

    # Высота коллажа — основной акцент
    card_top = 210
    card_bottom = 1710
    card_h = card_bottom - card_top
    collage_h = card_h - header_h - caption_block_h - pad
    collage_h = max(720, collage_h)

    # Пересчёт card_h под фактический контент и центрирование
    ...
    d = ImageDraw.Draw(card)
    brand_label = brand
    bb = d.textbbox((0, 0), brand_label, font=brand_font)
    bh = bb[3] - bb[1]
    d.text((ax + avatar_size + 16, (header_h - bh) // 2 - 2), brand_label, font=brand_font, fill=TEXT_FG)
    ...
    for img, (x, y, w, h) in zip(images, cells):
        ...
        # лёгкое скругление ячеек
        tile_rgba = tile.convert("RGBA")
        _paste_rounded(card, tile_rgba, (x, y), radius=10)
    ...
    sdraw.rounded_rectangle((20, 24, card_w + 20, card_h + 24), radius=40, fill=(0, 0, 0, 70))
    shadow = shadow.filter(ImageFilter.GaussianBlur(radius=16))
    bg.alpha_composite(shadow, (card_x - 20, card_top - 20))

    mask = _rounded_mask((card_w, card_h), 36)
    ...
    logger.info(
        "Story frame built (bubble): title=%r price=%r brand=%r font=%s",
        title_text,
        price_text,
        brand,
        _FONT_SOURCE,
