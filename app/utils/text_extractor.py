...
def extract_model_and_price(text: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    if not model_name:
        ...
        model_patterns = [
            r'(iPhone\s+\d+\s*(?:Pro\s*Max|Pro|Plus|mini|e)?\s+\d+Gb\s+\w+)',
...
def extract_model_name_without_code(text: str) -> Optional[str]:
    """
    Извлекает название модели из текста поста без цифрового кода.
    
    Примеры:
    - "iPhone 12 mini 128Gb Purple 5982" -> "iPhone 12 mini 128Gb Purple"
    - "iPhone 15 128Gb Black 3486" -> "iPhone 15 128Gb Black"
    
    Args:
        text: Текст поста
        
    Returns:
        Название модели без цифрового кода или None
    """
    model_name, _ = extract_model_and_price(text)
    
    if not model_name:
        return None
    
    # Удаляем цифровой код в конце (4-5 цифр, обычно после названия модели)
    # Паттерн: пробел + 4-5 цифр в конце строки
    model_name = re.sub(r'\s+\d{4,5}\s*$', '', model_name).strip()
    
    # Также удаляем цифровой код, который может быть в скобках в конце
    model_name = re.sub(r'\s*\(\d{4,5}\)\s*$', '', model_name).strip()
